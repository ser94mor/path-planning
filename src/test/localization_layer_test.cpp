...
TEST_CASE("LocalizationLayer::GetCars", "[localization_layer]") {
  ...
  SECTION("LocalizationLayer::GetCars returns cars array of length 1 when sensor_fusion array with 1 entry provided") {
    ...
    REQUIRE(localization_layer.GetCars()[0] ==
            Car::FromVectorAssumingConstantVelocityAndLaneKeeping(car_vect_1, 0.0, config) );

    localization_layer.Update({car_vect_2}, 1.0);
    REQUIRE(localization_layer.GetCars().size() == 1 );
    REQUIRE(localization_layer.GetCars()[0] ==
            Car::FromVectorAssumingConstantVelocityAndLaneKeeping(car_vect_2, 1.0, config) );
  }
  ...
  SECTION("LocalizationLayer::GetCars returns cars array of len. 2 when sensor_fusion array with 2 entries provided") {
    ...
    REQUIRE(localization_layer.GetCars()[0] ==
            Car::FromVectorAssumingConstantVelocityAndLaneKeeping(car_vect_1, 0.0, config) );
    REQUIRE(localization_layer.GetCars()[1] ==
            Car::FromVectorAssumingConstantVelocityAndLaneKeeping(car_vect_2, 0.0, config) );
    ...
    REQUIRE(localization_layer.GetCars()[0] ==
            Car::FromVectorAssumingConstantVelocityAndLaneKeeping(car_vect_2, 1.0, config) );
    REQUIRE(localization_layer.GetCars()[1] ==
            Car::FromVectorAssumingConstantVelocityAndLaneKeeping(car_vect_1, 1.0, config) );
  }
}
