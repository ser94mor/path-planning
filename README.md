...
In this project, highway path planner is designed that is responsible for generating smooth jerk-minimising
trajectories that guide the car with the average velocity higher than that of the surrounding cars. The car
behaves very much like a real driver who is in hurry but does not want to violate the speed limit and
safety rules. The path planner is capable of planing lane changes to place a car in the best lane in terms of 
...
1. The car uses a perfect controller and will visit every (x,y) point it receives in the list every 0.02 seconds. 
...
2. There will be some latency between the simulator running and the path planner returning a path, 
with optimized code usually its not very long maybe just 1-3 time steps. 
During this delay the simulator will continue using points that it was last given.
...
forwarding it to the `Localization Layer`. It also retrieves car trajectory information in a form of an array
of `Car` objects for the successive points in time, this is, a planned trajectory, and converts it into 
X and Y trajectories in Cartesian frame. Then it gives this X/Y trajectories to the simulator.
...
cars positions at some time in the future. It relies on the assumption that the surrounding cars do not change 
their lanes and move with constant velocities. Although it is not exactly true, and some cars might change 
their lanes and slightly variate their velocities, it happens so rarely that it is not needed to utilize a
...
some time in the future for each possible next car state. It then weights all the planned cars and chooses the
...
For each possible next state for the `Car` object, `Behavior Layer` constructs 
future `Car` objects considering the positions and velocities of the surrounding cars. For *KeepLane* state it
plans to position the ego car slightly behind the car ahead. If there is no car ahead, it just sets to car's velocity
to a reasonable maximum velocity and plans car's position accordingly. For *LaneChange*\* state, the `Behavior Layer`
plans to position the ego car slightly behind the other car in intended lane 
that is currently violates its safety buffer. If no car
violates the safety buffer of the ego car, the `Behavior Layer` sets the car's velocity the a reasonable maximum 
and plans accordingly. The described procedure is a bit simplified, the code actually considers more subtleties.
...
- given the alternatives, the car should not stay in the right most lane because simulator might falsely declare
  the car out of lane.  (It is a workaround needed to cover differences in the `Simulator`'s method of 
...
trajectory between the current `Car` object and the `Car` object planned by the `Behavior Layer` and for collision 
avoidance. The `Trajectory Layer` maintains its own array of trajectory `Car` objects (points) generated during
...
all the code involved into the planning of the next `Car` object each time the information exchange between 
the `Path Planner` and the `Simulator` happens. 

The X/Y trajectories in the `Path Planner` cannot be changed.
The length of X/Y trajectory, this is, the duration of time, in the `Path Planner` is comparable to the smallest 
human drivers reaction time. The `Trajectory Layer` keeps much longer trajectory, and, if there is an unexpected
obstacle ahead, it forgets all the planned trajectory and asks the `Behavior Layer` to replan the future car position
given the new circumstances. The `Behavior Layer` "thinks" about 0.1 second before producing the future `Car` object
...
one on the picture below. There is a car ahead that moves with constant velocity. And there is a car behind 
in the neighboring lane that moves with exactly the same velocity. For such a case, the path planned will keep the ego
following the car ahead forever, until there is enough room to change lane. While human driver that in hurry in this 
case would considerably lower the speed and will change two lanes. Luckily, in real life and in the Udacity simulator 
...
in accordance with Gaussian distribution. Then it should evaluate all the trajectories and choose the one that meets
all the constraints, like it is done in 
...
It will allow to increase the maximum speed allowed by 2-3 miles per hour.
